...
router.post('/article-image', authController.protect, authController.restrictTo(ROLES.ADMIN), upload.single('image'), resizeArticleImage, (req, res) => {
    res.status(200).json({
        status: 'success',
...
router.post('/user-photo', authController.protect, upload.single('photo'), resizeUserPhoto, (req, res) => {
    res.status(200).json({
        status: 'success',
